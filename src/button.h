 ...
 * button.h
 *
 * Botão simples desenhado com primitivas da SDL (SDL_RenderRect /
 * SDL_RenderFillRect), com três estados visuais (neutro, hover, clicado)
 * conforme pede o enunciado (Itens 5 e 6).
 */
...
typedef struct {
    ...
    /* Textura de texto cacheada; recriada apenas quando o rótulo muda,
     * para não recriar a textura a cada frame. */
    SDL_Texture *label_texture;
    int label_texture_w;
    int label_texture_h;
    char label_texture_source[64]; /* último texto usado para gerar a textura */
} Button;
...
/* Atualiza o texto do botão (ex.: "Equalizar" <-> "Ver original"). */
